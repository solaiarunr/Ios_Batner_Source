...
//        var results: [RadarAddress] = []
    ...
    func configUI() {
        ...
        self.setLocationButton.backgroundColor = UIColor(named: "ShadowGreen")
        self.setLocationButton.cornerMiniumRadius()
        self.searchView.cornerViewMiniumRadius()
        self.NoteLbl.config(color: UIColor(named: "appblackcolor"), font: UIFont(name: APP_FONT_REGULAR, size: 15), align: .left, text: "Notetxt")
    ...
    func setupDropDown() {
        ...
        dropDown.selectionAction = { [weak self] index, item in
            ...
            let selectedItem = self.results[index]
            ...
            if let position = selectedItem["position"] as? [String: Any] {
                latitude = "\(position["lat"] as? Double ?? 0)"
                longitude = "\(position["lon"] as? Double ?? 0)"
            }
            
            if let regions = selectedItem["regionalStructure"] as? [[String: Any]] {
                for region in regions {
                    
                    let type = region["type"] as? String ?? ""
                    ...
                    if type == "regional.country" {
                        country = region["name"] as? String ?? ""
                        countryCode = region["isoCode"] as? String ?? ""
                    }
                }
            }
    ...
    @objc func textChanged() {
        ...
        guard let text = searchTextField.text, text.count >= 3 else {
            dropDown.hide()
            return
        }
        autocompleteWorkItem?.cancel()
        ...
        let workItem = DispatchWorkItem { [weak self] in
            guard let self = self else { return }
            let MAPY_API_KEY = "ZnflL3FNax7-2vGEFEDAirA91BVwJCHE3KrGJlukpfk"
            let query = text.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? ""
            let MAX_RESULTS = 2
         let locality =  UserDefaultModule.shared.getcountrycode() ?? "IN"
            print("localitychklocalitychk",locality)
            let urlString = "https://api.mapy.com/v1/suggest?apikey=\(MAPY_API_KEY)&query=\(query)&limit=\(MAX_RESULTS)&type=regional.address,regional.street,regional.municipality,regional.region,regional.country&lang=en&locality=\(locality)"
            ...
            URLSession.shared.dataTask(with: request) { data, response, error in
                ...
                do {
                    ...
                    self.results = items
                    // 🔹 UPDATED PART
                    let addresses = items.map { item -> String in
                    ...
                    DispatchQueue.main.async {
                        // 🔒 Prevent dropdown if text changed or cleared
                        guard self.searchTextField.text == text,
                              let currentText = self.searchTextField.text,
                              currentText.count >= 3 else {
                            self.dropDown.hide()
                            return
                        }
                        
                        self.dropDown.dataSource = addresses
...
extension MapViewController: UITextFieldDelegate {
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        ...
        //        dropDown.hide()                    // ✅ Optional: hide dropdown
        return true
    }
